...
import { switchMap, tap, map, filter, mergeAll } from 'rxjs/operators';
  ...
  getActiveOrdersIb(page: number) {
    ...
    if (uid) {
      ...
      group$.pipe(
        ...
        tap((query: any) => {
          console.log(`Querying order 1 for ${query}...`);
        }),
        switchMap((entry: any) => {
          if (entry.length > 0) {
            return entry.map((cd: any) => {
              var streams = Object.keys(cd.legs).map((legId: any) => {
                ...
                return leg$.pipe(switchMap(
                  (x: any) => {
                    if (x && x.hasOwnProperty('brokerOrderId')) {
                      ...
                      .doc(this.SelectedBrokerAccount.id).collection(this.SelectedBrokerAccount.id).doc(x.brokerOrderId).valueChanges();
                      return activeOrder$.pipe(
                        filter(value => value !== null && value !== undefined),
                        switchMap(
                        (doc3: any) => {
                          x.orderStatus = doc3.orderStatus;
                          x.orderTime *= 1000;
                          return of(x);
                        }
                      ));
              ...
              return combined$.pipe(map((x: any) => {
                cd.legs = x.filter((value: null) => value !== null);
                return cd;
              }))
            }); 
          }
        ...
        next: (x: any) => {
          if (x) {
            this.ActiveOrderList[x.groupId] = x;
          }
